...
import { Heart, MessageCircle, Repeat2, Plus, Send, Loader2, X, Eye, ChevronLeft, Search } from 'lucide-react'
...
export function ClipMeTab({ onClose, initialVideoId, initialUserId }: ClipMeTabProps) {
  ...
  const [currentView, setCurrentView] = useState<'feed' | 'search' | 'channel'>('feed')
  ...
  const [channelOverlayVideo, setChannelOverlayVideo] = useState<ClipMeVideo | null>(null)
  const [channelAvatarLightbox, setChannelAvatarLightbox] = useState<string | null>(null)
  ...
  const channelTotalLikes = useMemo(
    () => (channelData?.videos ?? []).reduce((sum, video) => sum + (video.likesCount ?? 0), 0),
    [channelData?.videos]
  )
  ...
  }, [channelSearchQuery, currentView])
  ...
  useEffect(() => {
    ...
    if (!filteredFeedVideos.length) return
    const exists = filteredFeedVideos.some(video => video.id === initialVideoId)
    if (!exists) return
    setActiveVideoId(initialVideoId)
    const node = videoRefs.current[initialVideoId]
    if (node) node.scrollIntoView({ block: 'start' })
  }, [initialVideoId, filteredFeedVideos])
  ...
  useEffect(() => {
    ...
    filteredFeedVideos.forEach(video => {
  ...
  useEffect(() => {
    if (!filteredFeedVideos.length) {
      setActiveVideoId(null)
      return
    }
    if (!activeVideoId || !filteredFeedVideos.some(video => video.id === activeVideoId)) {
      setActiveVideoId(filteredFeedVideos[0]?.id ?? null)
    }
  }, [filteredFeedVideos, activeVideoId])
  ...
  const publishVideo = async () => {
    ...
    setUploadProgress(0)
    try {
      const uploaded = await clipMeAPI.uploadClipVideo(uploadFile, undefined, percent => {
  ...
  return (
    <div className="h-full min-h-0 md:fixed md:inset-0 md:z-40 md:flex md:items-center md:justify-center md:bg-black md:p-0">
          ...
          {filteredFeedVideos.map(video => {
            ...
            return (
              <section
                ...
                // Prefer dvh; minHeight keeps a 100vh fallback only in browsers that don't understand dvh.
      ...
      {currentView === 'search' && (
        <div className="h-full min-h-0 flex flex-col bg-white dark:bg-[#15151a] text-black dark:text-white md:w-[min(560px,100vw)] md:h-full md:border-x md:border-black/[0.08] dark:md:border-white/[0.08] overflow-hidden">
          <div className="h-14 px-4 flex items-center gap-3 border-b border-black/[0.08] dark:border-white/[0.08] bg-white dark:bg-[#15151a] shrink-0">
            <button
              onClick={() => { setCurrentView('feed'); setChannelSearchQuery(''); setChannelSearchResults([]) }}
              className="h-9 w-9 rounded-full flex items-center justify-center hover:bg-black/[0.06] dark:hover:bg-white/[0.08]"
            >
              <ChevronLeft className="h-5 w-5" />
            </button>
            <Input
          ...
          <div className="flex-1 min-h-0 overflow-y-auto">
            {isChannelSearchLoading && <p className="text-sm text-black/60 dark:text-white/60 px-4 py-4">Поиск...</p>}
            {!isChannelSearchLoading && channelSearchQuery.trim().length > 0 && channelSearchResults.length === 0 && (
              <p className="text-sm text-black/60 dark:text-white/60 px-4 py-4">Каналы не найдены</p>
            )}
            {channelSearchResults.map(channel => (
              <button
                key={channel.id}
                onClick={() => { void openChannelFromSearch(channel.id) }}
                className="w-full flex items-center gap-3 px-4 py-3 hover:bg-black/[0.05] dark:hover:bg-white/[0.08] text-left"
              >
      ...
      {currentView === 'channel' && activeChannelUserId !== null && (
        <div className="h-full min-h-0 flex flex-col bg-black/95 text-white md:w-[min(560px,100vw)] md:h-full md:border-x md:border-white/10 overflow-hidden">
          <div className="h-full flex flex-col">
                ...
                <div className="rounded-2xl bg-white/10 p-3.5 space-y-3">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3 min-w-0">
                      <Avatar
                        className={cn('h-14 w-14', channelData.user.avatarUrl && 'cursor-zoom-in')}
                        onClick={() => {
                          if (channelData.user.avatarUrl) setChannelAvatarLightbox(channelData.user.avatarUrl)
                        }}
                      >
                      ...
                      <div className="min-w-0">
                        <p className="font-semibold truncate inline-flex items-center gap-1">
                          ...
                          {channelData.user.isBadgeVerified && <VerifiedBadge className="h-3 w-3 min-h-3 min-w-3" />}
                        </p>
                  ...
                  <div className="mt-1 grid grid-cols-4 gap-2 text-center text-xs">
                    ...
                    <div className="rounded-lg bg-white/10 py-2">
                      <p className="font-semibold">{channelTotalLikes}</p>
                      <p className="text-white/70">Лайки</p>
                    </div>
                    <div className="rounded-lg bg-white/10 py-2">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {(channelTab === 'videos' ? (channelData.videos ?? []) : (channelData.reposts ?? [])).map(video => (
                      <button
                        ...
                        onClick={() => openVideoFromChannel(video)}
                      >
            ...
            <Dialog open={!!channelOverlayVideo} onOpenChange={open => { if (!open) setChannelOverlayVideo(null) }}>
              <DialogContent showCloseButton={false} className="max-w-3xl w-[96vw] h-[92vh] bg-black border-black p-0 overflow-hidden">
                {channelOverlayVideo && (
                  <div className="relative h-full">
                    <button
                      onClick={() => setChannelOverlayVideo(null)}
                    ...
                    <CustomVideoPlayer
                      src={channelOverlayVideo.videoUrl}
                      className="h-full w-full"
                      autoPlay
                      loop
                      fit="contain"
            ...
            <Dialog open={!!channelAvatarLightbox} onOpenChange={open => { if (!open) setChannelAvatarLightbox(null) }}>
              <DialogContent className="bg-black/90 border-0 max-w-3xl p-2 flex items-center justify-center">
                {channelAvatarLightbox && (
                  <img
                    src={channelAvatarLightbox}
                    alt="Полный размер аватарки канала"
          ...
          <div className="space-y-3">
            ...
            {isUploading && (
              <div className="space-y-1">
                <div className="h-2 rounded-full bg-black/[0.08] dark:bg-white/[0.12] overflow-hidden">
                  <div
                    className="h-full bg-[#5d6cf5] transition-all duration-150"
                    style={{ width: `${uploadProgress}%` }}
                  />
                </div>
                <p className="text-xs opacity-70 text-right">{uploadProgress}%</p>
              </div>
            )}
